...
\subsection{Инструкция для работников студии}
...
Так же, страница заказов разделена на 3 части, в зависимости от статуса заказа: новые заказы, заказы в работе и завершённые заказы. Это позволяет работнику быстро ориентироваться в текущих задачах. На этой странице присутствует кнопка быстрой смены статуса заказа, без необходимости заходить в детали заказа или редактировать заказ. Однако, система запросит причину редактирования заказа, но ее можно оставить пустой. Все статусы заказов представлены на рисунке \ref{fig:worker_order_statuses}. Окно комментария представлено на рисунке \ref{fig:worker_order_status_comment}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/worker_order_statuses.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/worker_order_status_comment.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/worker_statistic_page.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/worker_forecast.png}
...
Алгоритм экспорта один и тот же, при желании можно экспортировать любую сущность, которая есть в системе. Для демонстрационного примера были выбраны заказы, филаменты, пользователи и принтеры. Для примера экспортируем филаменты в форме \textit{Excel}. После выбора сущности и формата, необходимо нажать кнопку "Экспортировать", после чего система сформирует файл и предложит его скачать. Пример экспортированного файла с филаментами в формате \textit{Excel} представлен на рисунке \ref{fig:worker_data_import_example}.
...
\subsection{Инструкция для клиентов}
